...
Для обучения скрапера должно быть достаточно файла
...
parent_url_id  - идентификатор родительской url с которой заходили на эту url
anchor - текст в элементе который можно кликать
to_click - True если нужно кликать этот элемент для перехода к целевой странице

Дополнительно в папке с данными есть исходные данные с помощью которых получался anchor_out.csv
...
parent_url - родительская url с которой заходили на эту url. -1 означает что нет родительской url
...
Дополнительно, после обработки сформирован файл pages_out.csv
Он отличается от файла pages.csv только одним дополнительным полем для удобства

parent_url_id - идентификатор родительской url с которой заходили на эту url/
                -1 означает что нет родительской url

Структура папок в каталоге pages:

название папки соответствует коду банка.
имена файлов в папках это url_id

в файле url_id без расширения хранится загруженная страница

Файлы вида url_id_anchors.csv - генерируются на основе данных из pages.csv
и соответствующего файла с загруженной страницей вида url_id

Поля в файле url_id_anchors.csv:  
...
Из этих данных формируется обучающий набор в файле anchor_out.csv

В настоящий момент сохранены пути, и страницы двух банков ведущие к страницам с информацией об их отделениях.
А так же сгенерирован обучающий набор anchor_out.csv
